...
model_path = '/root/autodl-tmp/NWPU-MOC-sx/exp/08-17_12-48_MOC_RS_MCC_loss_Mse_loss_alpha_1_base_lr_5e-05_NIR=True/all_ep_185_cls_avg_mae_4.3_cls_avg_mse_10.5_cls_weight_mse_34.2.pth'
...
#################################################################################
    # 存储单样本所有类别拼接结果（GT+Pred）
    sample_class_plots = []  
    # 先处理输入图像（RGB 为例，可根据需求换 NIR 或拼接多通道）
    rgb_np = rgb.data.cpu().numpy().squeeze().transpose(1, 2, 0)  # (H,W,C)
    rgb_np = (rgb_np - rgb_np.min()) / (rgb_np.max() - rgb_np.min() + 1e-5)
    rgb_np = (rgb_np * 255).astype(np.uint8)
    sample_class_plots.append(rgb_np)  # 输入图像放最上方
#################################################################################    
    for c_idx in range(len(categorys)):
        ...
        # 输出可视化结果到指定文件
        save_pred_path = "./vis/pred_map/" + str(index) + categorys[c_idx] + ".jpg"
        save_gt_path = "./vis/gt_map/" + str(index) + categorys[c_idx] + ".jpg"
        print("save_pred_path:",save_pred_path)
        # print("save_gt_path:",save_gt_path)
        
        vis_img = pred_map[:,c_idx,:,:]
        vis_img = np.squeeze(vis_img)
        # print("vis_image:",vis_img.shape)
        # normalize density map values from 0 to 1, then map it to 0-255.
        vis_img = (vis_img - vis_img.min()) / (vis_img.max() - vis_img.min() + 1e-5)
        vis_img = (vis_img * 255).astype(np.uint8)
        vis_img = cv2.applyColorMap(vis_img, cv2.COLORMAP_JET)
        #cv2.imwrite(save_pred_path, vis_img)

        # gt 可视化
        gt_vis_dmap = gauss_map[:,c_idx,:,:]
        gt_vis_dmap = np.squeeze(gt_vis_dmap)
        gt_vis_dmap = (gt_vis_dmap - gt_vis_dmap.min()) / (gt_vis_dmap.max() - gt_vis_dmap.min() + 1e-5)
        gt_vis_dmap = (gt_vis_dmap * 255).astype(np.uint8)
        gt_vis_dmap = cv2.applyColorMap(gt_vis_dmap, cv2.COLORMAP_JET)
        
        pred_map_text = str(round(pred_cnt,1))
        ...
        font_thickness = 1
        # 获取文本尺寸
        pred_map_text_size, _ = cv2.getTextSize(pred_map_text, font, font_scale, font_thickness)
        gt_text_size, _ = cv2.getTextSize(gt_text, font, font_scale, font_thickness)
        # 计算文本位置
        text_x = 0
        text_y = height - pred_map_text_size[1]

        # 添加文本到图像
        cv2.putText(vis_img, pred_map_text, (text_x, text_y), font, font_scale, (255, 255, 255), font_thickness,
                            cv2.LINE_AA)

        cv2.putText(gt_vis_dmap, gt_text, (text_x, text_y), font, font_scale, (255, 255, 255), font_thickness,
                        cv2.LINE_AA)
        
        cv2.imwrite(save_pred_path, vis_img)
        cv2.imwrite(save_gt_path, gt_vis_dmap)
...
def test(net, test_loader):
    ...
    for index, data in enumerate(test_loader, 0):
        ...
        with torch.set_grad_enabled(False):
            ...
            # print("pred_map:",pred_map.shape) # (1,6,128,128)
