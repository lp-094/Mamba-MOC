...
from models.Mamba.vmamba import Backbone_VSSM
from models.Mamba.vmamba_scale import Test_VSSM
from models.Mamba.customized_model import SCBlock
from models.Mamba.vmamba_conv import Conv_VSSM
...
class SCAModule(nn.Module):
    def __init__(self, inn, out):
        super(SCAModule, self).__init__()
        self.conv_cat = Conv2d(inn, out, 1, same_padding=True, bn=False)
...
class MCC(nn.Module):
    ...
    def __init__(self, num_class=6,  pretrained=True):
        ...
        self.seen = 0
        self.fusion = cfg.MM # 若cfg.MM为True，则启用多模态
        if self.fusion:
            in_chans = 4  # RGB(3) + NIR(1) = 4通道
        else:
            in_chans = 3  # 仅RGB

        # 添加打印语句，验证通道数
        print(f"Backbone 输入通道数: {in_chans}") 
    
        #初始化 backbone 时指定输入通道数
        self.Extractor = Backbone_VSSM(in_chans=in_chans)  # 新增in_chans参数
        
        if pretrained:
            self.Extractor.init_weights('/root/autodl-tmp/NWPU-MOC-sx/pretrained_models/vssm_base_0229_ckpt_epoch_237.pth')
        ...
        self.neck = FPN()
        self.project_in = 896
        self.prog = SCBlock(dim=self.project_in)
        self.project_out = 64
        self.reduce = nn.Conv2d(self.project_in, self.project_out, 1, 1)

        self.output_layer = SCAModule(self.project_out, 6)
        
        self.multi_scale_mamba = Test_VSSM(dim=self.project_out, layer_num=2)
        self.conv_mamba = Conv_VSSM(dim=self.project_out, layer_num=2)
    ...
    def forward(self,rgb, nir):
        ...
        nir.shape = (b,1,h,w)
        '''
        #print("输入NIR通道数:", nir.shape[1])  # 应输出1
        if self.fusion:
            # 拼接RGB（3通道）和NIR（1通道），结果为4通道
            x = torch.cat([rgb,nir], dim = 1)  # dim=1 是通道维度，拼接后 shape: (B, 4, H, W)
        else:
            x = rgb
        #print(f"拼接后输入图像通道数: {x.shape[1]}")  # 输出应为4（多模态）或3（单模态）
        
        # x = rgb
        # print("x:",x.shape)
        x = self.Extractor(x)
        # for item in x:
        #     print("extract_x:",item.shape)
        x = self.neck(x)
        # for ittem in x:
        #     print("neck_x:",ittem.shape)
        x = torch.cat([F.interpolate(x[0],scale_factor=0.5), x[1],F.interpolate(x[2],scale_factor=2)], dim=1)
        # print("cat_x:",x.shape)
        # x = self.prog(x)
        x = self.reduce(x)
        # print("reduce_x:",x.shape)
        x = self.conv_mamba(x)
