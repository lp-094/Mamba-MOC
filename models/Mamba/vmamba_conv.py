...
class SS2Dv2:
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        # 计算卷积部分
        B,C,H,W = x.shape
        x_conv = self.conv2d_1_1(x) + self.conv2d_3_3(x)
        x_conv1 = x_conv.reshape(B,4,1,H,W)
        x_conv1 = x_conv1.reshape(B,4,1,H*W)
        conv_out = self.conv2d_1_2(x_conv).permute(0,2,3,1)
        y = self.forward_core(x, x_conv1) + conv_out
        
        # y = self.forward_core(x, x_conv1)
        y = self.out_act(y)
        if not self.disable_z:
            y = y * z
        out = self.dropout(self.out_proj(y))
...
class Conv_VSSM(nn.Module):
    def __init__(self, dim=256, layer_num=2):
        super().__init__()
        self.dim = dim
        self.layer_num = layer_num
        self.mamba_f = VSSBlock(
                hidden_dim=dim,
                drop_path=0.0,
